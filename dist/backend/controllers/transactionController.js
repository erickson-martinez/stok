...
const transactionController = {
    async createSimple(req, res) {
        try {
            const { ownerPhone, type, name, amount, date, status, notes, } = req.body;
    ...
    async createControlled(req, res) {
        try {
            const { ownerPhone, counterpartyPhone, name, amount, date, notes, type } = req.body;
            ...
            const controlId = `ctrl-${Date.now()}-${crypto_1.default.randomUUID().slice(0, 8)}`;
            const transactionDate = new Date(date);
            if (type === 'revenue') {
                const mySide = new Transaction_1.default({
                ...
                const counterpartySide = new Transaction_1.default({
    ...
    async updatePaymentStatus(req, res) {
        try {
            ...
            if (transaction.isControlled && transaction.controlId) {
                ...
                await Transaction_1.default.updateOne({ controlId: transaction.controlId, type: oppositeType }, {
    ...
    async listTransactions(req, res) {
        try {
            ...
            if (status)
                query.status = status;
            let transactions = await Transaction_1.default.find(query)
                .sort({ date: -1 })
                .lean();
            if (includeShared) {
                const shared = await Transaction_1.default.find({
            ...
            const previousTransactions = await Transaction_1.default.find(accumulatedQuery).lean();
    ...
    async followUser(req, res) {
        try {
            ...
            const result = await Transaction_1.default.updateMany({ ownerPhone: encryptedTarget }, { $set: { sharerPhone: encryptedMy, aggregate } });
            res.json({
    ...
    async deleteTransaction(req, res) {
        try {
            ...
            if (transaction.isControlled && transaction.controlId) {
                await Transaction_1.default.deleteMany({ controlId: transaction.controlId });
                res.json({ message: 'Transação controlada e sua contraparte foram removidas' });
            }
            else {
                await Transaction_1.default.findByIdAndDelete(transactionId);
    ...
    async subtractValue(req, res) {
        try {
            ...
            transaction.additions = transaction.additions || [];
            const index = transaction.additions.findIndex(item => !item.removed && item.description.trim().toLowerCase() === description.trim().toLowerCase());
            if (index === -1) {
                res.status(400).json({
